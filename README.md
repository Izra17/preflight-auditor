...
## ⚠️ Read this first: what could and couldn't be executed in this delivery

This project was built in a sandboxed development environment **with no outbound network
access** — `npm install`, `playwright install`, and any live request to `neemans.com` are
all impossible from inside that sandbox. Concretely, that means:

- Every file in this repository is complete, real, non-stub source code — nothing here is a
  placeholder or a TODO for "core functionality."
- **`npm install`, the TypeScript build, and a live audit against neemans.com could not be
  executed by the assistant that wrote this code.** I cannot claim to have run them, so I
  won't pretend to.
- `/samples/sample-audit.json` and `/samples/sample-report.md` are **explicitly labeled
  fixture/demo data** (as this assignment's own instructions require when live execution
  isn't available) — hand-authored against the real `AuditResult` schema to show the exact
  shape and tone of a genuine report, not fabricated as if they came from a live run.
- What *is* exercised without a network or a browser — deterministic tracking
  classification, duplicate detection, scoring, URL/SSRF validation, the rule engine — is
  covered by real unit tests in `/tests`, runnable with `npm test` once dependencies are
  installed on a machine with network access.

**To get a genuine live audit of neemans.com:** run `npm install && npx playwright install
chromium && npm run dev` on a machine with internet access, open `http://localhost:3000`,
and paste a real product or collection URL from neemans.com. See "Running a live Neeman's
audit" below for a fuller walkthrough.
...
If `ANTHROPIC_API_KEY` is unset, `generateAiInsight()` returns `null` immediately. The UI
...
at "duplicate GA4 page_view" + "GTM container present" + "direct gtag.js snippet also
...
- "Safe to interact with" means: clicking a CTA and an add-to-cart control is acceptable;
  ...
  information is not, and is explicitly skipped with a `NOT_TESTED` result rather than
  silently omitted.
...
- Navigation and interaction timeouts are enforced throughout (`AUDIT_NAV_TIMEOUT_MS`,
  `AUDIT_TIMEOUT_MS`), and a small in-process concurrency gate bounds how many Chromium
  instances can run at once (`AUDIT_MAX_CONCURRENCY`).
- Only an allowlisted set of query-parameter keys are ever surfaced in evidence
  (`SAFE_PARAM_ALLOWLIST` in `detectors.ts`); everything else — session IDs, emails, cart
...
docker run -p 3000:3000 -e ANTHROPIC_API_KEY=sk-ant-... preflight
```
...
|---|---|---|
| `ANTHROPIC_API_KEY` | No | Enables the optional AI interpretation layer |
| `ANTHROPIC_MODEL` | No | Defaults to `claude-sonnet-4-6` |
| `AUDIT_TIMEOUT_MS` | No | Hard ceiling on total audit time (default 45000) |
| `AUDIT_NAV_TIMEOUT_MS` | No | Navigation timeout (default 20000) |
| `AUDIT_MAX_CONCURRENCY` | No | Max concurrent audits per process (default 2) |
| `PREFLIGHT_STORE` | No | `memory` (default) or `file` for JSON-file-backed history |
...
e-commerce PDP, bounded by `AUDIT_TIMEOUT_MS`.
...
cp .env.example .env.local   # optional: add ANTHROPIC_API_KEY to enable AI insights
npm run dev
```
...
page_view, failed tracking request, broken JS, missing conversion event. The Playwright
...
- **AI-assisted, optional, requires `ANTHROPIC_API_KEY`:** executive summary language,
...
- Not run end-to-end by the author due to the sandboxed, network-disabled development
  environment (see the notice at the top of this file).
